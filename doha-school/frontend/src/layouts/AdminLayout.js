...
import {
  FiHome, FiUsers, FiUser, FiFileText, FiCalendar, FiImage,
  FiMail, FiBell, FiLogOut, FiMenu, FiX, FiSun, FiMoon,
  FiChevronRight, FiSettings, FiExternalLink
} from 'react-icons/fi';
    ...
    <div className="flex flex-col h-full">
      ...
      <div className="p-6 border-b border-gray-100 dark:border-gray-700">
        <Link to="/" className="flex items-center gap-3">
          ...
          <div>
            <div className="font-display font-bold text-primary-800 dark:text-white text-sm">Doha-Model</div>
      ...
      <nav className="flex-1 p-4 space-y-1 overflow-y-auto">
        ...
        <div className="pt-4 border-t border-gray-100 dark:border-gray-700 mt-4">
          ...
          <Link to="/" target="_blank"
            ...
            <FiExternalLink size={18} /><span>View Website</span>
          </Link>
          <button onClick={() => dispatch(toggleTheme())} className="admin-sidebar-link w-full text-left">
      ...
      <div className="p-4 border-t border-gray-100 dark:border-gray-700">
        <div className="flex items-center gap-3 p-3 rounded-xl bg-gray-50 dark:bg-gray-700 mb-3">
          ...
          <div className="flex-1 min-w-0">
            <p className="font-medium text-gray-800 dark:text-white text-sm truncate">{admin?.name || 'Admin'}</p>
            <p className="text-xs text-gray-400 truncate capitalize">{admin?.role || 'admin'}</p>
          </div>
        </div>
          ...
          <>
            ...
            <motion.aside initial={{ x: -280 }} animate={{ x: 0 }} exit={{ x: -280 }}
              ...
              <div className="absolute top-4 right-4">
                <button onClick={() => setSidebarOpen(false)} className="p-2 rounded-xl bg-gray-100 dark:bg-gray-700 text-gray-600 dark:text-gray-300">
                  <FiX size={18} />
                </button>
              </div>
              <SidebarContent />
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        ...
        <header className="bg-white dark:bg-gray-800 border-b border-gray-100 dark:border-gray-700 px-4 lg:px-6 py-4 flex items-center justify-between">
          <div className="flex items-center gap-4">
            <button onClick={() => setSidebarOpen(true)} className="lg:hidden p-2 rounded-xl text-gray-600 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-700">
              <FiMenu size={20} />
            </button>
            <div>
              <h1 className="text-lg font-display font-bold text-gray-900 dark:text-white">Admin Dashboard</h1>
              <p className="text-xs text-gray-400 hidden sm:block">Doha-Model School Management System</p>
            </div>
          </div>
          <div className="flex items-center gap-3">
            <button onClick={() => dispatch(toggleTheme())} className="p-2 rounded-xl text-gray-500 hover:bg-gray-100 dark:hover:bg-gray-700 transition-colors">
              {isDark ? <FiSun size={18} /> : <FiMoon size={18} />}
            </button>
