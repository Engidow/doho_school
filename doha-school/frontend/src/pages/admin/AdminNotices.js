      ...
      {loading ? (
        ...
        <div className="space-y-4">
          {notices.map((n) => (
            <div
              key={n._id}
              className={`card p-5 flex items-start gap-4 ${!n.isPublished ? "opacity-60" : ""}`}
            >
              <div
                className={`w-10 h-10 rounded-xl flex items-center justify-center flex-shrink-0 ${n.category === "urgent" ? "bg-red-100 dark:bg-red-900/30 text-red-500" : "bg-primary-100 dark:bg-primary-900/30 text-primary-500"}`}
              >
                <FiBell size={18} />
              </div>
              <div className="flex-1 min-w-0">
                <div className="flex flex-wrap items-center gap-2 mb-1">
                  ...
                  <span
                    className={`badge ${CAT_COLORS[n.category] || "badge-blue"} capitalize`}
                  >
                    {n.category}
                  </span>
                  {!n.isPublished && (
